...
public class AdminBatchView extends VerticalLayout implements View, EntityPushListener<Batch> {
	 ...
	 * @param batchHisto
	 * @return */
	private String getLabelBatchHisto(final BatchHisto batchHisto) {
	...
	private String getTimeFormated(final Object t) {
		try {
			if (t == null) {
				return "";
			} else {
	...
	public String getLabelProgramme(final Batch batch) {
		String label = "";

		if (batch.getFixeYearBatch() != null && batch.getFixeMonthBatch() != null && batch.getFixeDayBatch() != null) {
			label += applicationContext.getMessage("batch.prog.jour", new Object[] {
					DateTimeFormatter.ofPattern("dd/MM/yyyy").format(LocalDate.of(batch.getFixeYearBatch(), batch.getFixeMonthBatch(), batch.getFixeDayBatch()))}, UI.getCurrent().getLocale());
		} else if (batch.getFixeMonthBatch() != null && batch.getFixeDayBatch() != null) {
			label += applicationContext.getMessage("batch.prog.annuel", new Object[] {batch.getFixeDayBatch(),
	...
	@Override
	public void entityPersisted(final Batch entity) {
	...
	@Override
	public void entityUpdated(final Batch entity) {
		batchTable.removeItem(entity);
		entity.setLastBatchHisto(batchController.getLastBatchHisto(entity));
		batchTable.addItem(entity);
		batchTable.sort();
	...
	@Override
	public void entityDeleted(final Batch entity) {
